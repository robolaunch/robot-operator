...
func (r *RobotIDEReconciler) reconcileCheckService(ctx context.Context, instance *robotv1alpha1.RobotIDE) error {
	...
	if err != nil {
		...
		if instance.Spec.Ingress {
			...
			instance.Status.ServiceStatus.URLs["code-server-filebrowser"] = robotv1alpha1.GetRobotServiceDNS(*robot, "https://", "/file-browser/ide/")
			...
			instance.Status.ServiceStatus.URLs["code-server-filebrowser"] = robotv1alpha1.GetRobotServiceDNSWithNodePort(*robot, "http://", strconv.Itoa(int(serviceQuery.Spec.Ports[1].NodePort)))
		}
	}
